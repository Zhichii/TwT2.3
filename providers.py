...
class OpenAIProvider:
    ...
    def _to_format(self, conversation : msgs.Conversation):
        cur = 0
        messages = []
        while conversation.conversation[cur].child is not None:
            cur = conversation.conversation[cur].children[conversation.conversation[cur].child]
            msg = conversation.conversation[cur]
            if (msg.type == "MsgBase"):
        ...
        return messages
    def __call__(self, model : str, conversation : msgs.Conversation, max_tokens : int = 4096, stream : bool = True):
        messages = self._to_format(conversation)
...
class AnthropicProvider:
    ...
    def _to_format(self, conversation : msgs.Conversation):
        ...
        return (messages, "\n".join(system), )
    def __call__(self, model : str, conversation : msgs.Conversation, max_tokens : int = 4096, stream : bool = True):
        messages, system = self._to_format(conversation)
